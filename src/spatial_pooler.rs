...
impl SpatialPooler {
    ...
    pub fn adapt_synapses(&mut self, input_vector: &[bool]) {
        ...
        for column in &self.winner_columns {
            for val in self.potential_pool.connections_by_index_mut(*column).iter_mut() {
                ...
                //self.update_inputs.insert(val.index as usize);
            }
            self.potential_pool.update_permanences_for_column(*column, true, (self.stimulus_threshold + 0.5) as i32, &self.syn_perm_options);
        }
    }
    ...
    pub fn update_duty_cycles(&mut self) {
        ...
        for (index, &val) in self.overlaps.iter().enumerate() {
            self.overlap_duty_cycles[index] = ((self.overlap_duty_cycles[index] * (period - 1.0)) + (val > 0.0) as usize as f32) / period;
        }
        ...
        for &val in self.winner_columns.iter() {
             self.active_duty_cycles[val] = ((self.active_duty_cycles[val] * (period - 1.0)) + 1.0) / period;
        }
       
    }

    pub fn bump_up_weak_columns(&mut self) {
        for ((column, overlap_duty_cycle), min_overlap_duty_cycle) in self.overlap_duty_cycles.iter().enumerate().zip(self.min_overlap_duty_cycles.iter()) {
    ...
    pub fn update_boost_factors(&mut self) {
        ...
        if (got_elements) {
            for ((boost, &min_active), &active) in self.boost_factors.iter_mut().zip(self.min_active_duty_cycles.iter()).zip(self.active_duty_cycles.iter()) {
                *boost = if active > min_active {
                    1.0
                } else {
                    let ma = if min_active == 0.0 { 
                        1.0
                    } else {
                        min_active
                    };
    ...
    pub fn update_min_duty_cycles_global(&mut self) {
        let m = self.min_pct_overlap_duty_cycles * *self.overlap_duty_cycles.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let m2 = self.min_pct_active_duty_cycles * *self.active_duty_cycles.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
    ...
    {
        for val in &mut self.overlaps {
            *val = 0.0;
        }
        
        /*
        for (in_index,&val) in input_vector.iter().enumerate() {
            if val {
                for &col_index in self.potential_pool.connections_by_input(in_index, true) {
                   self.overlaps[self.potential_pool.column_global_index_to_column(col_index)] += 1.0;
                }
            }   
        }
        */
        ...
        for (in_index,&val) in input_vector.iter().enumerate() {
            if val {
                for &col_index in self.potential_pool.connections_by_input(in_index, false) {
                   if (self.potential_pool.connection_by_global_index(col_index).permanence >= self.syn_perm_options.connected) {
                        self.overlaps[self.potential_pool.column_global_index_to_column(col_index)] += 1.0;
                   }
                }
            }   
    ...
    pub fn inhibit_columns(&mut self) {
        ...
        if density <= 0.0 {
            let inhibitionArea = cmp::min((2 * self.inhibition_radius + 1).pow(self.column_dimensions.len() as u32), self.num_columns as usize);
        ...
        if self.global_inhibition || self.inhibition_radius > *self.column_dimensions.iter().max().unwrap() {
     ...
     pub fn inhibit_columns_local(&mut self, density: f32) {
        ...
        for (column, &overlaps) in self.overlaps.iter().enumerate() {
            if overlaps >= stimulus_threshold  {
               let neighborhood = self.column_topology.neighborhood(column, inhibition_radius, self.wrap_around);
    ...
    pub fn connect_and_configure_inputs(&mut self) {
       ...
       //let mut arr = vec![0usize; self.max_potential()];
        ...
        for i in 0..self.num_columns {
            ...
            self.potential_pool.setup_pool(&self.columns[i], &arr[range.clone()], self.init_connected_pct, &self.syn_perm_options, &mut self.rand);
            self.potential_pool.update_permanences_for_column(i, true, (self.stimulus_threshold + 0.5) as i32, &self.syn_perm_options);
        }       
        //self.potential_pool.test_connections();
        self.potential_pool.setup_input_pool(&self.syn_perm_options);
    ...
    pub fn map_potential(&mut self,
        ...
        let elements_iter = self.input_topology.neighborhood(center_input, self.potential_radius as usize, wrap_around);
    ...
    pub fn update_inhibition_radius(&mut self) {
        if self.global_inhibition {
            ...
            for column in &self.columns {
                ...
                for syn in cs {
                   for (index, val) in self.input_topology.compute_coordinates(syn.index as usize).enumerate() {
                ...
                total += min.iter().zip(max.iter()).fold(0, |acc, (x,y)| acc + y - x + 1) as f64 / size as f64;
            }
            total /= self.columns.len() as f64;

            let avg: f64 = self.column_dimensions.iter().zip(self.input_dimensions.iter()).map(|(&c,&i)| c as f64 / i as f64).sum::<f64>() / self.column_dimensions.len() as f64;
    ...
    pub fn gen_potential_pool(&mut self) {
       ...
       self.potential_pool = PotentialPool::new(self.num_columns, self.num_inputs, self.num_inputs);
    }
        ...
        self.input_topology
            .index_from_coordinates(self.column_topology
                                      ...
                                      .map(|((index, &col_dim), &in_dim)| {
                                               let new_index =
                                                   ((index as f32 / col_dim as f32) * in_dim as f32 +
                                                    (in_dim as f32 / col_dim as f32) * 0.5f32) as
                                                   usize;
    ...
    pub fn sample_into<T, I, R>(rng: &mut R, iterable: I, amount: usize, into: &mut [T]) -> std::ops::Range<usize>
    ...
    pub fn sample_into_universal<T : Copy, I, R>(rng: &mut R, iterable: I, amount: usize, into: &mut [T]) -> std::ops::Range<usize>
        where I: IntoIterator<Item = T>,
              R: Rng
    {
        let mut iter = iterable.into_iter();
        ...
        let final_size =  if items < amount {
            items
        } else {
            amount
        };
