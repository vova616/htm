...
fn main3() { 
    ...
    let mut classifier: SDRClassifier<u8> = SDRClassifier::new(vec![0,1], 0.1, 0.3, sp.num_columns);
           ...
           //println!("{:?}", sp.winner_columns);
            ...
            if i == 99 {
                ...
                for &(ref step, ref probabilities) in &r {
                    println!("{} {:?}", step, probabilities.iter().enumerate().max_by(|&(_,a), &(_,b)| a.partial_cmp(b).unwrap()).unwrap());
                }
            }
...
fn main() {
    ...
    for _ in 0..images.size {
        ...
        let r = classifier.compute(record as u32, label as usize, label as u8, &sp.winner_columns[..], true, true);
        record += 1;
    }
    ...
    for i in 0..test_images.size {
        ...
        //println!("{:?}", &sp.winner_columns[..]);
        let r = classifier.compute(record as u32, label as usize, label as u8, &sp.winner_columns[..], false, true);
        for &(ref step, ref probabilities) in &r {
            let (answer, score) = probabilities.iter().enumerate().max_by(|&(_,a), &(_,b)| a.partial_cmp(b).unwrap()).unwrap();
    ...
    println!("Accuracy: {}, Total: {} Good: {}", good as f32 / total as f32, total, good);
}
